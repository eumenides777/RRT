...
RRT_RecordCount = 0
RecordCount = 0
touchsensor0  = flapper.getDevice('touchsensor0')
touchsensor1  = flapper.getDevice('touchsensor1')
touchsensor0.enable(1)
touchsensor1.enable(1)
### Desired trajectory
p_d_x   =   0 
p_d_x_last = 0
p_d_y   =   0
...
goal = [15, 12]
    # 障碍物 (x, y, radius)
obstacle_list = []
rrt = RRT(rand_area=[0, 18], obstacle_list=obstacle_list, max_iter=300)
path = rrt.rrt_planning(start=[0, 0], goal=[15, 15], animation=show_animation)
...
distance = 0
distance_list =[]
def plot_path( start_t, end_t, num_points=100):
    t_values = np.linspace(start_t, end_t, num_points)
    x_y_values = []
        

    for t in t_values:
        (x, y) = rrt.return_p_d_x_d_y(t)
            
        x_y_values.append((x,y))
    return x_y_values
rrt_x_y_list = plot_path(1,T,num_points=100)
#print(rrt_x_y_list)
print(T)
data = {'rrt_list':rrt_x_y_list}
scio.savemat('rrt_list_'+str(rrt_count)+'.mat',data)
data = {'obstacle_list':obstacle_list}
scio.savemat('obstacle_list'+str(rrt_count)+'.mat',data)
#with open("rrt.list0", "wb") as file:
   # pickle.dump(rrt_x_y_list, file)     
# Create the robot instance

# Get the display instance
#display = Display('display')
# Set the display mode
#display.setColor(0xFFFFFF)  # Set color to white
#display.setFont('Arial',5,False)  # Set font to default

# Draw a 3D line on the display
#start_point = [0.0, 0.0, 0.0]
#end_point = [1.0, 1.0, 1.0]
#display.drawLine(int(start_point[0]),int(start_point[1]), int(end_point[0]), int(end_point[1]))
RRT_coordinate=[]
# Run the simulation
Time_interval =[]
...
while flapper.step(timestep) != -1:
    a=touchsensor1.getValue()
    b=touchsensor0.getValue()
    

    while a ==1.0 or b==1.0:
        record_touch_time = record_touch_time+1
        Time_interval.append(RecordCount)
        if Time_interval[record_touch_time] - Time_interval[record_touch_time-1] <1000:
            break
        p_x = x_coordinates[RecordCount-1]
        p_y = y_coordinates[RecordCount-1]
        p_x_before = x_coordinates[RecordCount-2]
        p_y_before = y_coordinates[RecordCount-2]
        v_x = p_x -  p_x_before
        v_y = p_y -  p_y_before
        square_sum = math.sqrt(v_x**2 + v_y**2)
        x = v_x/square_sum
        y = v_y/square_sum
        goal_x = x_coordinates[RecordCount-1000]
        goal_y = y_coordinates[RecordCount-1000]
        #obstacle_list.append((p_x+x, p_y+y, 1))
        obstacle_list.append((p_x+0.7, p_y+0.7, 1))
        rrt.obstacle_list =  obstacle_list
        
        rrt_count = rrt_count+1
        path = rrt.rrt_planning(start=[goal_x, goal_y], goal=[15, 15], animation=show_animation) 
        #if show_animation and path:
           # plt.show()
        rrt.cal_xishu()
        T = rrt.cal_xishu()
        T = RecordCount/1000 +T 
        #T = path_plan.cal_xishu()
        rrt_x_y_list = plot_path(1,T,num_points=1000)
        data = {'rrt_list':rrt_x_y_list}
        scio.savemat('rrt_list'+str(rrt_count)+'.mat',data)  
        data = {'obstacle_list':obstacle_list}
        scio.savemat('obstacle_list'+str(rrt_count)+'.mat',data)
        a=0.0 
        b=0.0
    ...
    print (Flapper_Rotation_current_T)
   ...
   ####-------------Control Tasks-------------------
    ...
    x_coordinates.append(Flapper_translation_value[0])
    y_coordinates.append(Flapper_translation_value[1])
    #print('sb=',x_coordinates[RecordCount])
   # print(type(x_coordinates[RecordCount]))
    #print(Flapper_pos)
    #print(type(Flapper_translation_value[0]))
    ...
    #p_d_x   = (-1)  *  np.cos ( np.pi  * RecordCount * Simulation_Gap ) + k_rate 
    #p_d_y   =     np.sin ( np.pi  * RecordCount * Simulation_Gap )
   
    #v_d_x   =      np.pi  * np.sin ( np.pi  * RecordCount * Simulation_Gap )
    #v_d_y   =      np.pi  * np.cos ( np.pi  * RecordCount * Simulation_Gap )
    
    #(p_d_x,p_d_y)   = rrt.return_p_d_x_d_y (  (RRT_RecordCount) * Simulation_Gap )  
    
    
    
   
    #(v_d_x,v_d_y)   =   rrt.return_v_d_x_d_y (  (RRT_RecordCount) * Simulation_Gap )
    #print(p_d_x,p_d_y)
    #print('速度为',v_d_x,v_d_y)
   # d_v_d_x = 0
    # = 0
    #distance = math.sqrt((p_d_x -Flapper_translation_value[0] )**2 + (p_d_y - Flapper_translation_value[1])**2)
    #d_v_d_x =     np.pi *  np.pi  * np.cos ( np.pi  * RecordCount * Simulation_Gap )
    #d_v_d_y = (-1)*  np.pi  * np.pi  * np.sin ( np.pi  * RecordCount * Simulation_Gap )
    ...
    p_d     = np.mat([  [float(p_d_x)],    [float(p_d_y)], [float(p_d_z)]])
    v_d     = np.mat([  [float(v_d_x)],    [float(v_d_y)], [float(v_d_z)]])
    d_v_d   = np.mat([  [float(d_v_d_x)],  [float(d_v_d_y)], [float(d_v_d_z)]])
    
    
    
    
    Torward_direction = np.mat([  [1], [0], [0]])
    Flight_direction  = np.mat([  [float(v_d_x)], [float(v_d_y)], [0]])
    ...
    ###sinusoidal   
        ...
        temp_u_squre = u_t.T * u_t
        num = float(temp_u_squre[0,0])
        
        
        StrokeFreq =  11 / 9.8* num **0.5
    ...
    if (RecordCount < 1000*T):
        ...
        RRT_coordinate.append((p_d_x,p_d_y))
    ...
    if (RecordCount >= 1000*T):
        ...
        scio.savemat(file_name+Total_File, { 'Total_body_translation_list':Total_body_translation_list,\
                                  'RRT_coordinate':RRT_coordinate,\
                                  'distance_list':distance_list})#'Total_rudder_torque':Total_rudder_torque_list,\
                                  #'Total_wing_torque':Total_wing_torque_list,\
        ...
        #np.save('Total_body_translation_list',Total_body_translation_list)
    ...
    RRT_RecordCount = RRT_RecordCount+1
    print(RRT_RecordCount)
    if (RecordCount>1000*T):
        break
